...
/**
 * 
 *  READ UCRF15 LOGICAL DESCENDING BY conditionList THEN
...
public class PaternReadByLogicalDescending extends AbstractPatternFromXToYWithoutCarriageReturn {
	...
	AbstractToken astViewName,astKeywordWith,astLogical,astDescending;

	public PaternReadByLogicalDescending() {
		super();
		...
		// LOGICAL
		astLogical = new OzelKelimeToken("LOGICAL_DESCENDING", 0, 0, 0);
		astLogical.setSourceFieldName("astLogical");
		patternTokenList.add(astLogical);
	...
	public AbstractCommand getmatchedCommand(List<AbstractToken> tokenListesi, int offset) {
		...
		if(!currentTokenForMatch.tokenMatchs(astLogical)){
			return null;
		}
		setTokenToElement(matchedCommand, currentTokenForMatch,astLogical);
		...
		while(tokenListIterator.hasNext()){
			...
			if(	currentTokenForMatch.getTip().equals(TokenTipi.OzelKelime)
					...
					&&!currentTokenForMatch.getDeger().equals(ReservedNaturalKeywords.LOGICAL)
