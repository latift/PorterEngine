...
public class JavaFindWithElement extends AbsctractConditionalJavaElement {
	...
	public boolean writeJavaToStream() throws Exception{
		...
		try {
			...
			JavaClassElement.javaCodeBuffer.append("List<"+pojoName+"> ");
...
private String createFindByMethodString() {
	...
	for(int index=0; index<conditionListWithFiltersAndParantesiz.size();index++){
		...
		if(curFilter.getFilterValue().getTip().equals(TokenTipi.Sayi)){
			...
			methodParameterType=ConvertUtilities.getVariableTypeOfString(curFilter.getFilterValue()).toString();
			methodParameterName=curFilter.getFilterValue().getLinkedToken().getDeger().toString().replaceAll("-","_");
		}else if(curFilter.getFilterValue().isPojoVariable()){
			methodParameterType=ConvertUtilities.getVariableTypeOfString(curFilter.getFilterValue().getColumnNameToken()).toString();
			methodParameterName=curFilter.getFilterValue().getColumnNameToken().getDeger().toString().replaceAll("-","_");
		}else{
			methodParameterType=ConvertUtilities.getVariableTypeOfString(curFilter.getFilterValue()).toString();
			methodParameterName=curFilter.getFilterValue().getDeger().toString().replaceAll("-","_");
		}
		findByString.append(methodParameterType+" "+ConvertUtilities.onlyFieldOfIncludedVariable(methodParameterName));
 ...
 */
				...
				if(curToken.getDeger().equals('(')){
					...
					if(index<conditionListWithFiltersAndParantesiz.size()-1){
						...
						if(nextElement instanceof AbstractToken){
							AbstractToken nextToken=(AbstractToken) nextElement;
							if(nextToken.isOneOfOzelKelime("OR","AND") || nextToken.isOneOfKelime("WHERE")){
								orCondition=true;
								break;
							}
						}
	...
	*/
	 ...
	 */
	private String createFindByString() throws Exception {
		StringBuffer findByString=new StringBuffer("findBy");
		AbstractToken curToken;
		Filter curFilter;
