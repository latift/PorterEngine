...
public class JavaProcedureDivision extends AbstractJavaElement{
	
	final static Logger logger = Logger.getLogger(JavaProcedureDivision.class);
	
	ConversionLogModel logModel;

	public JavaProcedureDivision() {
		super();
	...
	public StringBuilder writeJavaBaslat(String fullJavaFilePath) throws Exception {
		...
		try {
			JavaProcedureDivision.javaCodeBuffer= new StringBuilder(); //Her çağrıldığında önce buffer bir boşaltılmalı.
			this.writeJavaToStream();
			WriteToFile.writeToFile(JavaProcedureDivision.javaCodeBuffer,fullJavaFilePath);
		} catch (IOException e) {
			logger.info(e.getMessage(),e);
		}
		logger.info("End of Java Export");
		return JavaProcedureDivision.javaCodeBuffer;
