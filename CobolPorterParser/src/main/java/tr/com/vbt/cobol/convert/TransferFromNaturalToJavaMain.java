...
public class TransferFromNaturalToJavaMain {
	...
	private static boolean isInConversionSet(File file) {
		...
		if(ConversionLogModel.getInstance().getConversionMode().equals(ConversionMode.ALL_SOURCE_CODES)){
			return true;
		}else if(ConversionLogModel.getInstance().getConversionMode().equals(ConversionMode.SOURCE_CODES_WITHOUT_GENERATED_JAVA) && isJavaAlreadyGenerated(file)){
			logger.warn("Conversion Statu: "+ ConversionLogModel.getInstance().getFileName()+" has not been started cause of java is already there ");
			logger.debug(file.getName()+" Java Kodu Daha once yaratıldığından tekrar çalıştırılmadı. Çalıştırmak isteniyorsa output folderdan java dosyası silinmeli.");
			return false;
				
		}else if(ConversionLogModel.getInstance().getConversionMode().equals(ConversionMode.SOURCE_CODES_WITHOUT_GENERATED_LEX) && isLexAlreadyGenerated(file)){
			logger.warn("Conversion Statu: "+ ConversionLogModel.getInstance().getFileName()+" has not been started cause of lex is already there " );
			logger.debug(file.getName()+" Lex Aşamasına Daha once geldiğinden tekrar çalıştırılmadı. Çalıştırmak isteniyorsa output folderdan lex dosyası silinmeli.");
			return false;
		}
		return true;
	}
